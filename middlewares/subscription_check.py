...
class SubscriptionMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        if isinstance(
                event,
                types.CallbackQuery) and event.data == "refresh_public_status":
        ...
        if config.TEST_MODE:
            logging.info(
                f"Test mode is ON. Ignoring update from non-admin user {user.id if user else 'Unknown'}.")
            return
        ...
        try:
            ...
            logging.error(
                f"Ошибка проверки подписки: {e}. Убедитесь, что бот является администратором канала {self.channel_id}")
        except Exception as e:
            logging.error(
                f"Непредвиденная ошибка в SubscriptionMiddleware: {e}",
                exc_info=True)
