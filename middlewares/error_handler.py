...
def get_traceback_paginator_keyboard(error_id: str, page: int, total_pages: int):
    # ... (код без изменений)
    builder = InlineKeyboardBuilder()
    nav_buttons = []
    if page > 1:
        nav_buttons.append(InlineKeyboardButton(text="« 1", callback_data=f"error_page:{error_id}:1"))
        nav_buttons.append(InlineKeyboardButton(text="‹", callback_data=f"error_page:{error_id}:{page-1}"))
    nav_buttons.append(InlineKeyboardButton(text=f"· {page}/{total_pages} ·", callback_data="noop"))
    if page < total_pages:
        nav_buttons.append(InlineKeyboardButton(text="›", callback_data=f"error_page:{error_id}:{page+1}"))
        nav_buttons.append(InlineKeyboardButton(text=f"{total_pages} »", callback_data=f"error_page:{error_id}:{total_pages}"))
...
async def handle_errors(event: ErrorEvent, bot: Bot):
    ...
    logging.exception(f"Cause exception: {event.exception}\nUpdate: {event.update}")
    ...
    total_pages = math.ceil(len(error_cache[error_id]) / TRACEBACK_LINES_PER_PAGE)
    ...
    for admin_id in admins_to_notify:
        try:
            markup = get_traceback_paginator_keyboard(error_id, 1, total_pages) if total_pages > 1 else None
            await bot.send_message(chat_id=admin_id, text=full_admin_text, reply_markup=markup)
        except TelegramForbiddenError:
            logging.warning(f"Не удалось отправить уведомление об ошибке админу {admin_id}: пользователь не начал диалог с ботом")
        except Exception as e:
            logging.error(f"Не удалось отправить уведомление об ошибке админу {admin_id}: {e}")
