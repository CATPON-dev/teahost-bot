...
async def get_my_userbot_logs(
    ...
    return APIResponse(data={"logs": log_lines[-lines:], "total_lines": len(log_lines)})
...
async def create_userbot(
    ...
    server_ip = next((ip for ip, d in server_config.get_servers().items() if d.get("code") == server_code), None)
    ...
    asyncio.create_task(log_api_action("installation_via_api", {
        "user_data": {"id": tg_user_id, "full_name": current_user.get('full_name', str(tg_user_id))},
        "ub_info": {"name": ub_username, "type": ub_type},
        "server_info": {"ip": server_ip, "code": server_config.get_servers().get(server_ip, {}).get("code", "N/A")}
    }))
...
async def delete_userbot(request: Request, ub_username: str, current_user: dict = Depends(verify_token)):
    ...
    if not deletion_result.get("success"):
        logging.warning(f"Container deletion failed for {ub_username} but proceeding with DB cleanup. Error: {deletion_result.get('error')}")

    await db.delete_userbot_record(ub_username)
    
    asyncio.create_task(log_api_action("api_delete_userbot", {
    ...
    asyncio.create_task(api_delete_and_notify(tg_user_id, ub_username, server_ip, request.client.host))
...
async def manage_userbot(
    ...
    if action == "start": result = await api_manager.start_container(ub_username, server_ip)
    elif action == "stop": result = await api_manager.stop_container(ub_username, server_ip)
    elif action == "restart": result = await api_manager.restart_container(ub_username, server_ip)
