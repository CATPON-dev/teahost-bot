...
def get_session_check_keyboard(
        view_mode: str,
        page: int = 0,
        total_pages: int = 1):
    builder = InlineKeyboardBuilder()
    ...
    if total_pages > 1:
        ...
        if page > 0:
            nav_buttons.append(
                InlineKeyboardButton(
                    text="⏮️",
                    callback_data=f"check_page:{view_mode}:0"))
            nav_buttons.append(
                InlineKeyboardButton(
                    text="◀️",
                    callback_data=f"check_page:{view_mode}:{page-1}"))
        else:
            nav_buttons.append(
                InlineKeyboardButton(
                    text="⏮️",
        ...
        if page < total_pages - 1:
            ...
            nav_buttons.append(
                InlineKeyboardButton(
                    text="⏭️",
                    callback_data=f"check_page:{view_mode}:{total_pages-1}"))
        else:
            nav_buttons.append(
                InlineKeyboardButton(
                    text="▶️",
    ...
    if view_mode == 'suspicious':
        builder.button(
            text="✅ Показать нормальных",
            callback_data="check_view_toggle:normal")
    else:  # normal
        builder.button(
            text="⚠️ Показать подозрительных",
            callback_data="check_view_toggle:suspicious")

    builder.row(
        InlineKeyboardButton(
            text="🔄 Обновить",
            callback_data="refresh_session_check"))
...
def get_review_approval_keyboard(user_id: int, user_message_id: int):
    ...
    builder.button(
        text="✅ Да",
        callback_data=f"approve_review:{user_id}:{user_message_id}")
...
def get_server_selection_keyboard(
        user_id: int,
        installed_bots_map: dict,
        server_stats: dict,
        servers_on_page: list,
        page: int,
        total_pages: int,
    ...
    if total_pages > 1:
        ...
        if page > 1:
            nav_buttons.append(
                InlineKeyboardButton(
                    text="« 1",
                    callback_data=f"select_server_page:1"))
        if page > 1:
            nav_buttons.append(
                InlineKeyboardButton(
                    text="‹",
                    callback_data=f"select_server_page:{page - 1}"))
        ...
        if page < total_pages:
            nav_buttons.append(
                InlineKeyboardButton(
                    text=f"{total_pages} »",
                    callback_data=f"select_server_page:{total_pages}"))
    ...
    if is_chat:
        if has_bots:
            builder.button(
                ...
                callback_data="go_to_control_panel")
        else:
            builder.button(
                text="🚀 Установить юзербот",
                callback_data="create_userbot_start")
        builder.adjust(1)
        return builder.as_markup()
...
def get_api_token_keyboard(is_shown: bool):
    ...
    builder.button(
        text=toggle_text,
        callback_data="toggle_api_token_visibility")

    builder.button(
        text="🔄 Сгенерировать новый",
        callback_data="regenerate_api_token")
...
def get_user_list_paginator(page: int, total_pages: int, view_mode: str):
    ...
    if page > 1:
        nav_buttons.append(
            InlineKeyboardButton(
                text="⬅️ Назад",
                callback_data=f"user_page:{view_mode}:{page-1}"))
    if page < total_pages:
        nav_buttons.append(
            InlineKeyboardButton(
                text="Вперед ➡️",
...
def get_confirm_unstable_server_keyboard(server_ip: str):
    ...
    builder.button(
        text="✅ Да, продолжить",
        callback_data=f"confirm_unstable:{server_ip}")
    builder.button(
...
def get_select_ub_type_keyboard(server_ip: str):
    ...
    builder.button(
        text="🪐 Heroku",
        callback_data=f"ub_type:heroku:{server_ip}")
    builder.button(
        text="🦊 FoxUserbot",
        callback_data=f"ub_type:fox:{server_ip}")
    builder.button(
        text="🌙 Legacy",
        callback_data=f"ub_type:legacy:{server_ip}")
    builder.button(
        text="🔙 Назад к выбору сервера",
        callback_data="create_userbot_start")
    builder.adjust(1)
    return builder.as_markup()
...
def get_management_keyboard(
    ...
    if not is_server_active:
        builder.button(
            ...
            callback_data=f"noop:{owner_id_str}")
    elif is_running:
        builder.button(
            text="🔴 Выключить",
            callback_data=create_callback("stop"))
        builder.button(text="🔄 Перезагрузить",
                       callback_data=create_callback("restart"))
        ...
        builder.button(
            text="🔑 Данные для авторизации",
            callback_data=create_callback("auth"))
        builder.button(text="🇩🇪VPN", callback_data=create_callback("vpn"))
        builder.button(text="🔀 Переустановка",
                       callback_data=create_callback("recreate"))
    else:
        builder.button(
            text="🚀 Включить",
            callback_data=create_callback("start"))
    ...
    if is_owner:
        if is_private and not is_installing and not is_deleting:
            builder.row(
                InlineKeyboardButton(
                    text="🖥️ Терминал",
                    switch_inline_query_current_chat="exec "),
                InlineKeyboardButton(
                    text="⚡️ Inline действие",
                    switch_inline_query_current_chat="action "))
        if not is_inline:
            if is_private:
                builder.row(
                    InlineKeyboardButton(
                        text="📜 Логи",
                        callback_data=f"show_user_logs:docker:{ub_username}:{owner_id_str}:1"))
            if is_private:
                builder.row(
                    InlineKeyboardButton(
                        text="👥 Поделиться панелью",
                        callback_data=f"share_panel_start:{ub_username}"))
            if is_private:
                builder.row(
                    InlineKeyboardButton(
                        text="🔄 Сменить сервер",
                        callback_data=f"migrate_ub_start:{ub_username}:{owner_id_str}"))
            builder.row(
                InlineKeyboardButton(
                    text="🗑️ Удалить",
                    callback_data=f"delete_ub_confirm_request:{ub_username}:{owner_id_str}"))
    else:
        if not is_inline:
            builder.row(
                InlineKeyboardButton(
        ...
        if is_shared:
            builder.row(
                InlineKeyboardButton(
                    text="🚫 Отказаться от доступа",
                    callback_data=f"shared_reject_access:{ub_username}"))
    ...
    if hasattr(builder, 'inline_keyboard'):
        builder.inline_keyboard = [
            row for row in builder.inline_keyboard if row and any(
                b for b in row)]
    return builder.as_markup()


def get_migration_server_selection_keyboard(
        ub_username: str,
        owner_id: int,
        servers_list: list,
        installed_bots_map: dict,
...
def get_confirm_delete_keyboard(ub_username: str):
    ...
    builder.button(
        text="✅ Да, удалить",
        callback_data=f"delete_ub_execute:{ub_username}")
    builder.button(
        text="❌ Нет, отмена",
        callback_data=f"delete_ub_cancel:{ub_username}")
    return builder.as_markup()


def get_confirm_reinstall_keyboard(ub_username: str):
    builder = InlineKeyboardBuilder()
    builder.button(text="✅ Да, переустановить",
                   callback_data=f"reinstall_ub_execute:{ub_username}")
    builder.button(
        text="❌ Отмена",
        callback_data=f"reinstall_ub_cancel:{ub_username}")
    builder.adjust(1)
    return builder.as_markup()
...
def get_back_to_panel_after_reinstall(ub_username: str):
    builder = InlineKeyboardBuilder()
    builder.button(text="🔙 Назад к панели",
...
def get_agreement_keyboard():
    builder = InlineKeyboardBuilder()
    builder.button(
        text="📖 Почитать соглашение",
        url="https://telegra.ph/Polzovatelskoe-soglashenie-dlya-SharkHost-06-23")
    builder.button(
...
def get_public_status_keyboard(
        installed_bots_map: dict,
        server_stats: dict,
        servers_on_page: dict,
        page: int = 1,
    ...
    if total_pages > 1:
        ...
        if page > 1:
            nav_buttons.append(
                InlineKeyboardButton(
                    text="« 1",
                    callback_data=f"status_page:1"))
        if page > 1:
            nav_buttons.append(
                InlineKeyboardButton(
                    text="‹",
                    callback_data=f"status_page:{page - 1}"))
        nav_buttons.append(
            InlineKeyboardButton(
                text=f"· {page}/{total_pages} ·",
                callback_data="noop"))
        ...
        if page < total_pages:
            nav_buttons.append(
                InlineKeyboardButton(
                    text=f"{total_pages} »",
                    callback_data=f"status_page:{total_pages}"))
        builder.row(*nav_buttons)
    builder.row(
        InlineKeyboardButton(
            text="🔄 Обновить",
            callback_data="refresh_public_status"))
    return builder.as_markup()
...
def get_ub_info_keyboard(is_running: bool, ub_username: str, is_blocked: bool):
    ...
    if is_running:
        builder.button(
            text="⏹ Остановить",
            callback_data=f"manage_ub_info:stop:{ub_username}")
        builder.button(text="🔄 Перезапустить",
                       callback_data=f"manage_ub_info:restart:{ub_username}")
    else:
        builder.button(
            text="▶️ Запустить",
            callback_data=f"manage_ub_info:start:{ub_username}")
    builder.adjust(2)
    ...
    if is_blocked:
        builder.row(
            InlineKeyboardButton(
                text="🟢 Разблокировать",
                callback_data=f"toggle_block_ub:{ub_username}:0"))
    else:
        builder.row(
            InlineKeyboardButton(
                text="🚫 Заблокировать",
                callback_data=f"toggle_block_ub:{ub_username}:1"))

    builder.row(
        InlineKeyboardButton(
            text="📝 Добавить/Изменить заметку",
            callback_data=f"add_note_start:{ub_username}"))
    builder.row(
        InlineKeyboardButton(
            text="📜 Показать логи",
            callback_data=f"choose_log_type:{ub_username}"))
    return builder.as_markup()
...
def get_cancel_note_keyboard(ub_username: str):
    ...
    builder.button(
        text="❌ Отмена",
        callback_data=f"cancel_add_note:{ub_username}")
    return builder.as_markup()
...
def get_back_to_ub_panel_keyboard(ub_username: str):
    ...
    builder.button(
        text="🔙 Назад",
        callback_data=f"back_to_ub_info:{ub_username}")
    return builder.as_markup()
...
def get_log_type_choice_keyboard(ub_username: str, owner_id: int):
    ...
    builder.button(
        text="🐳 Docker",
        callback_data=f"show_logs:docker:{ub_username}:{owner_id}:1")
    builder.button(
        text="📄 Log File",
        callback_data=f"show_logs:logfile:{ub_username}:{owner_id}:1")
    builder.adjust(2)
    builder.row(InlineKeyboardButton(text="🔙 Назад к управлению",
                                     callback_data=f"back_to_ub_info:{ub_username}:{owner_id}"))
    return builder.as_markup()


def get_logs_paginator_keyboard(
        log_type: str,
        ub_username: str,
        page: int,
        total_pages: int,
    ...
    if page > 1:
        nav_buttons.append(
            InlineKeyboardButton(
                text="«",
                callback_data=f"show_logs:{log_type}:{ub_username}:{owner_id}:1"))
        nav_buttons.append(
            InlineKeyboardButton(
                text="‹",
                callback_data=f"show_logs:{log_type}:{ub_username}:{owner_id}:{page-1}"))
    nav_buttons.append(
        InlineKeyboardButton(
            text=f"{page}/{total_pages}",
            callback_data="noop"))
    if page < total_pages:
        ...
        nav_buttons.append(
            InlineKeyboardButton(
                text="»",
                callback_data=f"show_logs:{log_type}:{ub_username}:{owner_id}:{total_pages}"))
    builder.row(*nav_buttons)
    builder.row(
        InlineKeyboardButton(
            text="🔙 Назад к выбору логов",
            callback_data=f"choose_log_type:{ub_username}:{owner_id}"))
    return builder.as_markup()


def get_user_logs_paginator_keyboard(
        log_type: str,
        ub_username: str,
        page: int,
        total_pages: int,
    ...
    if page > 1:
        nav_buttons.append(
            InlineKeyboardButton(
                text="« 1",
                callback_data=f"show_user_logs:{log_type}:{ub_username}:{owner_id}:1"))
        nav_buttons.append(
            InlineKeyboardButton(
                text="‹",
                callback_data=f"show_user_logs:{log_type}:{ub_username}:{owner_id}:{page-1}"))
    ...
    if page < total_pages:
        ...
        nav_buttons.append(
            InlineKeyboardButton(
                text=f"{total_pages} »",
                callback_data=f"show_user_logs:{log_type}:{ub_username}:{owner_id}:{total_pages}"))
...
def get_cancel_transfer_keyboard(ub_username: str):
    ...
    builder.button(
        text="❌ Отмена",
        callback_data=f"transfer_ub_cancel:{ub_username}")
    return builder.as_markup()
...
def get_confirm_transfer_keyboard(ub_username: str, new_owner_id: int):
    ...
    builder.button(
        text="✅ Да",
        callback_data=f"transfer_ub_execute:{ub_username}:{new_owner_id}")
    builder.button(
        text="❌ Нет",
        callback_data=f"transfer_ub_cancel:{ub_username}")
    builder.adjust(2)
...
def get_confirm_reboot_keyboard(ip: str):
    builder = InlineKeyboardBuilder()
    builder.button(text="✅ Да, перезагрузить",
                   callback_data=f"host_reboot_confirm:{ip}")
...
def get_reinstall_keyboard():
    builder = InlineKeyboardBuilder()
    builder.button(
        text="🚀 Начать переустановку",
        callback_data="reinstall_userbot_start")
    return builder.as_markup()
...
def get_user_info_keyboard(user_id: int, has_bots: bool):
    ...
    if has_bots:
        builder.button(
            text="🤖 Юзерботы",
            callback_data=f"show_user_bots:{user_id}")
    return builder.as_markup()
...
def get_user_bots_list_keyboard(userbots: list, user_id: int):
    ...
    for bot in userbots:
        ...
        builder.button(
            text=f"🔹 {bot['ub_username']}{owner_mark}",
            callback_data=f"select_ub_panel:{bot['ub_username']}")
    builder.button(text="⬅️ Назад к информации",
                   callback_data=f"back_to_user_info:{user_id}")
    builder.adjust(1)
    return builder.as_markup()


def get_admin_ub_management_keyboard(
        ub_username: str,
        user_id: int,
        is_active: bool):
    builder = InlineKeyboardBuilder()
    ...
    if is_active:
        builder.button(
            text="🔴 Выключить",
            callback_data=f"admin_manage_ub:stop:{ub_username}:{user_id}")
        builder.button(
            text="🔄 Перезагрузить",
            callback_data=f"admin_manage_ub:restart:{ub_username}:{user_id}")
    else:
        builder.button(
            text="🚀 Включить",
            callback_data=f"admin_manage_ub:start:{ub_username}:{user_id}")

    builder.button(
        text="📜 Логи",
        callback_data=f"admin_show_logs:docker:{ub_username}:1")
    builder.button(
        text="🗑️ Удалить",
        callback_data=f"admin_delete_ub:{ub_username}")
...
def get_admin_logs_paginator_keyboard(
        log_type: str,
        ub_username: str,
        user_id: int,
        page: int,
    ...
    if page > 1:
        nav_buttons.append(
            InlineKeyboardButton(
                text="« 1",
                callback_data=f"admin_show_logs:{log_type}:{ub_username}:1"))
        nav_buttons.append(
            InlineKeyboardButton(
                text="‹",
                callback_data=f"admin_show_logs:{log_type}:{ub_username}:{page-1}"))
    ...
    if page < total_pages:
        ...
        nav_buttons.append(
            InlineKeyboardButton(
                text=f"{total_pages} »",
                callback_data=f"admin_show_logs:{log_type}:{ub_username}:{total_pages}"))

    builder.row(*nav_buttons)
    builder.row(
        InlineKeyboardButton(
            text="⬅️ Назад к управлению",
            callback_data=f"select_user_bot:{ub_username}:{user_id}"))
    return builder.as_markup()
...
def get_admin_cancel_transfer_keyboard(ub_username: str):
    ...
    builder.button(
        text="❌ Отмена",
        callback_data=f"admin_transfer_cancel:{ub_username}")
    return builder.as_markup()
...
def get_admin_confirm_transfer_keyboard(ub_username: str, new_owner_id: int):
    ...
    builder.button(
        text="✅ Да, передать",
        callback_data=f"admin_transfer_execute:{ub_username}:{new_owner_id}")
    builder.button(
        text="❌ Отмена",
        callback_data=f"admin_transfer_cancel:{ub_username}")
    builder.adjust(2)
...
def get_commits_list_keyboard(commits: list):
    ...
    for commit in commits:
        ...
        builder.button(
            text=button_text,
            callback_data=f"view_commit:{commit['commit_id']}")
    ...
    builder.row(
        InlineKeyboardButton(
            text="Скрыть",
            callback_data="hide_commits"))
    return builder.as_markup()


def get_commit_details_keyboard(
        commit_id: str,
        likes: int,
        dislikes: int,
        is_admin: bool):
    ...
    builder.button(
        text=f"👍 {likes}",
        callback_data=f"vote_commit:{commit_id}:1")
    builder.button(
        text=f"👎 {dislikes}",
        callback_data=f"vote_commit:{commit_id}:-1")
    ...
    if is_admin:
        builder.button(text="✍️ Редактировать",
                       callback_data=f"edit_commit_start:{commit_id}")
        builder.button(
            text="🗑️ Удалить",
            callback_data=f"delete_commit_start:{commit_id}")

    builder.row(
        InlineKeyboardButton(
            text="⬅️ Назад к списку",
            callback_data="back_to_commits"))
...
def get_commit_delete_confirm_keyboard(commit_id: str):
    ...
    builder.button(
        text="✅ Да, удалить",
        callback_data=f"delete_commit_confirm:{commit_id}")
    builder.button(
        text="❌ Нет",
        callback_data=f"delete_commit_cancel:{commit_id}")
    builder.adjust(2)
    return builder.as_markup()
...
def get_delub_reason_keyboard(ub_username: str, reasons: dict):
    ...
    for reason_code, reason_text in reasons.items():
        builder.button(
            text=reason_text.split('\n')[0],
            callback_data=f"delub_confirm:{ub_username}:{reason_code}")

    builder.adjust(1)
    builder.row(
        InlineKeyboardButton(
            text="❌ Закрыть",
            callback_data="delub_close_menu"))
    return builder.as_markup()
...
def get_delub_final_confirm_keyboard(ub_username: str, reason_code: str):
    ...
    builder.button(
        text="✅ Да, удалить",
        callback_data=f"delub_execute:{ub_username}:{reason_code}")
    builder.button(
        text="❌ Нет, назад",
        callback_data=f"delub_cancel:{ub_username}")
    builder.adjust(2)
    return builder.as_markup()
...
def get_confirm_revoke_shared_keyboard(ub_username: str):
    ...
    builder.button(
        text="✅ Да, отказаться",
        callback_data=f"confirm_revoke_shared:{ub_username}")
    builder.button(
        text="❌ Нет",
        callback_data=f"cancel_revoke_shared:{ub_username}")
    builder.adjust(2)
    return builder.as_markup()
...
def get_confirm_share_panel_keyboard(ub_username: str, share_user_id: int):
    ...
    builder.button(
        text="✅ Да, поделиться",
        callback_data=f"confirm_share_panel:{ub_username}:{share_user_id}")
    builder.button(
        text="❌ Нет",
        callback_data=f"cancel_revoke_shared:{ub_username}")
    builder.adjust(2)
...
def get_accept_share_panel_keyboard(ub_username: str, owner_id: int):
    ...
    builder.button(
        text="✅ Принять",
        callback_data=f"accept_share_panel:{ub_username}:{owner_id}")
    builder.button(
        text="❌ Отклонить",
        callback_data=f"decline_share_panel:{ub_username}:{owner_id}")
    builder.adjust(2)
    return builder.as_markup()
...
def get_retry_login_link_keyboard(ub_username: str):
    builder = InlineKeyboardBuilder()
    builder.button(text="🔄 Попробовать снова",
                   callback_data=f"retry_login_link:{ub_username}")
    builder.button(text="⬅️ Назад", callback_data="back_to_main_panel")
    return builder.as_markup()
...
def get_stats_keyboard(current_view: str = "overall"):
    ...
    builder.row(
        InlineKeyboardButton(
            text="🔄 Обновить",
            callback_data=f"stats_refresh:{current_view}"))
    return builder.as_markup()


def get_container_list_keyboard(
        page_containers: list,
        page: int,
        total_pages: int,
        expanded_container_name: str | None = None):
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton(
                text="⬅️ Назад",
                callback_data=f"container_page:{page-1}"))
    ...
    builder.row(
        InlineKeyboardButton(
            text="🔄 Обновить всё",
            callback_data="container_page:refresh"))
...
def get_api_config_paginator_keyboard(page: int, total_pages: int):
    ...
    if page > 1:
        nav_buttons.append(
            InlineKeyboardButton(
                text="⏮️ 1",
                callback_data=f"api_config_page:1"))
        nav_buttons.append(
            InlineKeyboardButton(
                text="◀️",
                callback_data=f"api_config_page:{page-1}"))
    ...
    if page < total_pages:
        ...
        nav_buttons.append(
            InlineKeyboardButton(
                ...
                callback_data=f"api_config_page:{total_pages}"))
...
def get_servers_paginator_keyboard(page: int, total_pages: int):
    ...
    if page > 1:
        nav_buttons.append(
            InlineKeyboardButton(
                text="⏮️ 1",
                callback_data=f"servers_page:1"))
        nav_buttons.append(
            InlineKeyboardButton(
                text="◀️",
                callback_data=f"servers_page:{page-1}"))
    ...
    if page < total_pages:
        ...
        nav_buttons.append(
            InlineKeyboardButton(
                text=f"{total_pages} ⏭️",
                callback_data=f"servers_page:{total_pages}"))
...
def get_confirm_delete_user_keyboard(user_id: int):
    ...
    builder.button(
        text="✅ Да, удалить",
        callback_data=f"deluser_db_confirm:{user_id}")
...
def get_cleanup_confirmation_keyboard(cleanup_id: str):
    ...
    builder.button(
        text="✅ Очистить",
        callback_data=f"cleanup_confirm:{cleanup_id}")
    builder.button(
        text="❌ Отменить",
