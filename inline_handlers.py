...
async def _get_full_server_info_text(stats_map, servers):
    ...
    for ip, details in servers.items():
        ...
        flags = []
        if details.get('hosting'): flags.append("☁️ Hosting")
        if details.get('vpn'): flags.append("🛡️ VPN")
        if details.get('proxy'): flags.append("🌐 Proxy")
...
async def inline_user_info_handler(inline_query: InlineQuery):
    ...
    if not identifier:
        result = InlineQueryResultArticle(
            ...
            input_message_content=InputTextMessageContent(message_text="ℹ️ Пожалуйста, укажите ID или юзернейм пользователя.")
        )
    ...
    if not user_data:
        result = InlineQueryResultArticle(
            ...
            input_message_content=InputTextMessageContent(message_text=f"❌ Пользователь <code>{html.quote(identifier)}</code> не найден в базе данных бота.", parse_mode="HTML")
        )
        await inline_query.answer([result], is_personal=True, cache_time=5)
        return
    ...
    reg_date_str = reg_date_obj.strftime('%d.%m.%Y в %H:%M') if isinstance(reg_date_obj, datetime.datetime) else "Неизвестно"
...
async def inline_action_handler(inline_query: InlineQuery, bot: Bot):
    ...
    # Проверка статуса юзербота - действия недоступны во время установки или удаления
    if ub_data.get('status') == 'installing':
        result = InlineQueryResultArticle(
    ...
    is_server_active = server_config.get_server_status_by_ip(the_only_bot['server_ip']) not in ["false", "not_found"]
    ...
    logging.info(f"Inline action check for {ub_username}: service is_running = {is_running} (from DB)")

    all_possible_actions = {
        "start": {"title": "🚀 Включить", "description": f"Запустить {ub_username}"},
        "stop": {"title": "🔴 Выключить", "description": f"Остановить {ub_username}"},
        "restart": {"title": "🔄 Перезагрузить", "description": f"Перезапустить {ub_username}"}
    }
...
async def inline_menu_handler(inline_query: InlineQuery):
    ...
    # Проверка статуса юзербота - меню недоступно во время установки или удаления
    if ub_data.get('status') == 'installing':
        result = InlineQueryResultArticle(
            id="menu_installing",
            title="⏳ Установка в процессе", 
