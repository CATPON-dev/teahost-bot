...
class _Config:
    ...
    def _load(self):
        ...
        if admin_ids is None:
            ...
            self.SUPER_ADMIN_IDS = [int(i)
                                    for i in admin_ids if isinstance(i, int)]
        else:
            self.SUPER_ADMIN_IDS = []
...
def add_super_admin(user_id: int) -> bool:
    ...
    if "admin_user_id" not in raw_config or not isinstance(
            raw_config["admin_user_id"], list):
...
def remove_super_admin(user_id: int) -> bool:
    if user_id not in config.SUPER_ADMIN_IDS:
        return False
    raw_config = _read_raw_config()
    if "admin_user_id" in raw_config and isinstance(
            raw_config["admin_user_id"], list):
        try:
