...
# Для определения операционной системы (например, для команды ping)
import platform
...
async def get_server_load(ip: str, ssh_user: str | None, ssh_pass: str | None, ssh_key_path: str | None = None) -> float:
    ...
    try:
        ...
        is_local_machine = (ip == "127.0.0.1" or ip ==
                            "localhost" or platform.node().lower() == ip.lower())

        if ssh_user is None and is_local_machine:
            # Если нет SSH-пользователя и это локальная машина, используем
            # psutil
            ...
            if ssh_key_path and os.path.exists(ssh_key_path):
                ...
                logger.warning(
                    f"Невозможно получить загрузку сервера для {ip}: отсутствуют SSH-учетные данные для удаленного сервера, и это не локальный хост.")
                return -1.0
        else:
            logger.warning(
                f"Невозможно получить загрузку сервера для {ip}: отсутствуют SSH-учетные данные для удаленного сервера, и это не локальный хост.")
...
async def get_server_specs(ip: str, ssh_user: str, ssh_pass: str, ssh_key_path: str | None = None):
    ...
    try:
        if not ssh_user:
            logger.warning(
                f"SSH-пользователь отсутствует для {ip}. Невозможно получить характеристики сервера.")
            return None

        # Общий и доступный диск
        disk_cmd = "df -h / | awk 'NR==2{print $2 \" \" $4}'"
        # Общая, использованная, свободная RAM
        ram_cmd = "free -m | awk 'NR==2{print $2 \" \" $3 \" \" $4}'"
        ...
        if ssh_key_path and os.path.exists(ssh_key_path):
            ...
            logger.warning(
                f"SSH-учетные данные отсутствуют для {ip}. Невозможно получить характеристики сервера.")
            return None
        ...
        logger.error(
            f"Не удалось получить характеристики сервера для {ip}: {e}")
        return None
...
async def get_server_stats(ip: str):
    ...
    try:
        ...
        # или если есть SSH-учетные данные (для сервера, который может не
        # отвечать на пинг, но доступен по SSH)
        if ping_success or (ssh_user and (ssh_pass or ssh_key_path)) or (
                ip == "127.0.0.1" or ip == "localhost" or platform.node().lower() == ip.lower()):
            load_task = get_server_load(ip, ssh_user, ssh_pass, ssh_key_path)
            # Запускаем получение спецификаций, только если есть SSH-учетные
            # данные
            specs_task = get_server_specs(
                ip,
                ssh_user,
                ssh_pass,
                ssh_key_path) if ssh_user and (
                ssh_pass or ssh_key_path) else asyncio.sleep(
                0,
                result=None)
            ...
            return {
                "ip": ip,
                "status": status,
                "cpu_load": f"{cpu_load:.1f}%" if cpu_load != -
                1.0 else "N/A",
                "specs": server_specs,
                "message": "Онлайн" if status == "online" else "Частичные данные"}
        else:
            # Если пинг не удался и это не локальный сервер, считаем его
            # оффлайн
            return {
                "ip": ip,
                "status": "offline",
...
async def manage_ub_service(ub_username: str, action: str, server_ip: str) -> dict:
    ...
    logger.info(
        f"Попытка {action} службы юзербота {ub_username} на {server_ip}")
    ...
    if not ssh_user:
        return {
            "success": False,
            "message": "Отсутствует SSH-пользователь для сервера."}
        ...
        if ssh_key_path and os.path.exists(ssh_key_path):
            async with asyncssh.connect(server_ip, username=ssh_user, client_keys=[ssh_key_path], known_hosts=None, timeout=10) as conn:
                ...
                if result.exit_status == 0:
                    logger.info(
                        f"Команда {action} для {service_name} на {server_ip} успешно отправлена")
                    return {
                        "success": True,
                        "message": f"Команда '{action}' успешно выполнена."}
                else:
                    logger.error(
                        f"Не удалось {action} {service_name} на {server_ip}: {result.stderr}")
                    return {
                        "success": False,
                        "message": f"Команда не выполнена: {result.stderr}"}
        elif ssh_pass:
            ...
            return {
                "success": False,
                "message": "Отсутствуют SSH-учетные данные для сервера."}
    except asyncssh.Error as e:
        logger.error(
            f"Ошибка SSH при управлении {service_name} на {server_ip}: {e}")
        return {"success": False, "message": f"Ошибка SSH-соединения: {e}"}
    except Exception as e:
        logger.error(
            f"Неожиданная ошибка при управлении {service_name} на {server_ip}: {e}")
...
async def get_ub_service_status(ub_username: str, server_ip: str) -> dict:
    ...
    logger.info(
        f"Проверка статуса службы юзербота {ub_username} на {server_ip}")
    creds = server_config.get_server_credentials(server_ip)
    ssh_user = creds.get("ssh_user")
    ssh_pass = creds.get("ssh_pass")
    ssh_key_path = creds.get("ssh_key_path")
    ...
    try:
        ...
        if ssh_key_path and os.path.exists(ssh_key_path):
            async with asyncssh.connect(server_ip, username=ssh_user, client_keys=[ssh_key_path], known_hosts=None, timeout=10) as conn:
                # check=False, так как может быть код выхода 1 для неактивного
                ...
                if status_text == "active":
                    return {
                        "status": "running",
                        "message": "Служба активна и запущена."}
                elif status_text == "inactive":
                    return {"status": "stopped",
                            "message": "Служба неактивна (остановлена)."}
                elif status_text == "failed":
                    return {"status": "error",
                            "message": "Служба находится в состоянии ошибки."}
                else:
                    return {
                        "status": "unknown",
                        "message": f"Неизвестный статус службы: {status_text}"}
        elif ssh_pass:
            async with asyncssh.connect(server_ip, username=ssh_user, password=ssh_pass, known_hosts=None, timeout=10) as conn:
                # check=False, так как может быть код выхода 1 для неактивного
                result = await conn.run(cmd, check=False)
        ...
        logger.error(
            f"Ошибка SSH при получении статуса для {service_name} на {server_ip}: {e}")
        return {"status": "error", "message": f"Ошибка SSH-соединения: {e}"}
    except Exception as e:
        logger.error(
            f"Неожиданная ошибка при получении статуса для {service_name} на {server_ip}: {e}")
...
async def create_server_user_and_setup_hikka(tg_user_id: int, ub_username: str, ub_type: str, server_ip: str) -> dict:
    ...
    """
    logger.info(
        f"Инициирование создания юзербота {ub_username} на {server_ip} для пользователя {tg_user_id}")
    creds = server_config.get_server_credentials(server_ip)
    ssh_user = creds.get("ssh_user")
    ssh_pass = creds.get("ssh_pass")
    ssh_key_path = creds.get("ssh_key_path")
    ...
    try:
        ...
        if ssh_key_path and os.path.exists(ssh_key_path):
            async with asyncssh.connect(server_ip, username=ssh_user, client_keys=[ssh_key_path], known_hosts=None, timeout=60) as conn:
                # 1. Создать пользователя на сервере (пример, заменить на
        ...
        # Заглушка для реального пути Hikka (это должно быть из скрипта
        # настройки)
        hikka_path = f"/home/{ub_username}/Hikka"

        return {
            "success": True,
            "message": "Процесс создания юзербота инициирован.",
            "hikka_path": hikka_path}

    except asyncssh.Error as e:
        logger.error(
            f"Ошибка SSH во время создания юзербота для {ub_username} на {server_ip}: {e}")
        return {
            "success": False,
            "message": f"Ошибка SSH-соединения или команды: {e}"}
    except Exception as e:
        logger.error(
            f"Неожиданная ошибка во время создания юзербота для {ub_username} на {server_ip}: {e}")
        return {"success": False, "message": f"Неожиданная ошибка: {e}"}
...
async def delete_userbot_full(ub_username: str, server_ip: str) -> dict:
    ...
    """
    logger.info(
        f"Инициирование удаления юзербота {ub_username} на {server_ip}")
    creds = server_config.get_server_credentials(server_ip)
    ssh_user = creds.get("ssh_user")
    ssh_pass = creds.get("ssh_pass")
    ssh_key_path = creds.get("ssh_key_path")
    ...
    try:
        ...
        try:
            ...
            await conn.run("sudo systemctl daemon-reload", check=True)
            logger.info(
                f"Служба systemd для {ub_username} остановлена и удалена.")
        except Exception as e:
            logger.warning(
                f"Не удалось остановить/отключить службу для {ub_username} (возможно, не существует): {e}")

        # 2. Удалить домашний каталог пользователя и самого пользователя
        cmd_delete_user_data = f"sudo rm -rf /home/{ub_username}"
        # Используем check=False, если каталог не существует
        await conn.run(cmd_delete_user_data, check=False)
        logger.info(f"Удален /home/{ub_username}")

        # -r также удаляет домашний каталог
        cmd_delete_user = f"sudo userdel -r {ub_username}"
        # Используем check=False, если пользователь не существует
        await conn.run(cmd_delete_user, check=False)
        logger.info(f"Пользователь {ub_username} удален с {server_ip}")

        return {
            "success": True,
            "message": "Юзербот успешно удален с сервера."}

    except asyncssh.Error as e:
        logger.error(
            f"Ошибка SSH во время удаления юзербота для {ub_username} на {server_ip}: {e}")
        return {
            "success": False,
            "message": f"Ошибка SSH-соединения или команды: {e}"}
    except Exception as e:
        logger.error(
            f"Неожиданная ошибка во время удаления юзербота для {ub_username} на {server_ip}: {e}")
        return {"success": False, "message": f"Неожиданная ошибка: {e}"}
